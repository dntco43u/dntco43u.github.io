...
## GitHub repository
[바로 가기](https://github.com/dntco43u/s6h7k8rv)

## host 구성

### 의존 패키지 설치
{{< tabpane text=true >}}
  {{% tab header="**distro**" disabled=true /%}}
  {{% tab header="RHEL" %}}
```sh
sudo dnf -y update && sudo dnf install -y bc
```
  {{% /tab %}}
  {{% tab header="openwrt" %}}
```sh
opkg update && opkg install bc coreutils-numfmt coreutils-date coreutils-stat
```
  {{< /tab >}}
{{< /tabpane >}}
...
{{< tabpane text=true >}}
  ...
  {{% tab header="RHEL" %}}
```sh
vi ~/.local/bin/utils.sh
```
  {{% /tab %}}
  {{% tab header="openwrt" %}}
```sh
vi /usr/sbin/utils.sh
```
  {{< /tab >}}
{{< /tabpane >}}
```sh
...
[^1]: https://github.com/dntco43u/s6h7k8rv/blob/main/utils.sh
